...
function create_recentcomments($args = '', $start = 0) {
	...
	if ($args['administrator'] == 'false') {
		global $table_prefix;
		$sql_administrator = " AND comment_author_email NOT IN (SELECT A1.user_email FROM $wpdb->users A1, $wpdb->usermeta A2 WHERE A1.ID = A2.user_id AND A2.meta_key = '" . $table_prefix . "capabilities' AND A2.meta_value LIKE '%administrator%')";
	}
	...
	if ($comments) {
		foreach ($comments as $comment) {
			...
			if ($comment->comment_type == 'pingback') {
				...
				$result .= '<li id="rc_item_' . ++$count . '" class="rc_item">' . $element_avatar . sprintf('<div class="rc_info"><span class="author_name">%1$s</span>' . rc_get_author_info() . '</div>', '<a rel="nofollow" href="'. get_permalink($comment->comment_post_ID) . '#comment-' . $comment->comment_ID . '">' . $comment->comment_author . '</a>') . '<div class="rc_excerpt">' . $comment_excerpt . '</div></li>';
			}
		...
		if ($has_newest) {
			$newest = '<div id="rc_newest"><a rel="nofollow" href="javascript:void(0);" onclick="RCJS.page(\'' . get_bloginfo('siteurl') . '\',\'' . $argsf . '\',0,\'' . $loading_text . '\');">' . __('&laquo; Newest', 'wp-recentcomments') . '</a></div>';
		}
		...
		if ($has_newer) {
			...
			$newer = '<div id="rc_newer"><a rel="nofollow" href="javascript:void(0);" onclick="RCJS.page(\'' . get_bloginfo('siteurl') . '\',\'' . $argsf . '\',' . $newer_start . ',\'' . $loading_text . '\');">' . __('&laquo; Newer', 'wp-recentcomments') . '</a></div>';
		}

		// Older 按钮
		$older = '';
		if ($has_older) {
			$older = '<div id="rc_older"><a rel="nofollow" href="javascript:void(0);" onclick="RCJS.page(\'' . get_bloginfo('siteurl') . '\',\'' . $argsf . '\',' . $older_start . ',\'' . $loading_text . '\');">' . __('Older &raquo;', 'wp-recentcomments') . '</a></div>';
		}
...
function rc_substring($str, $start=0, $len=50, $comment_id, $argsf, $comment_start, $loading_text, $code='UTF-8') {
	...
	if($code == 'UTF-8') {
		...
		$expand = ' <span class="rc_expand"><a rel="nofollow" href="javascript:void(0);" onclick="RCJS.detail(' . $comment_id . ',\'' . get_bloginfo('siteurl') . '\',\'' . $argsf . '\',' . $comment_start . ',\'' . $loading_text . '\');">' . __('&raquo;', 'wp-recentcomments') . '</a></span>';
		return $str . $expand;
	}
}
